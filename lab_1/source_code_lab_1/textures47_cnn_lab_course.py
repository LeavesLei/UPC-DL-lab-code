"""
@author:Leaves
@file: texture47_cnn_lab_course.py
@time: 2019/03/01
"""
import os
import numpy as np
...
def load_data(data_path='./dtd/images/'):
    # read classes name
    ...
    for i in range(class_len):
        ...
        img_name_list = os.listdir(os.path.join(os.getcwd(),'dtd', 'images',class_name))
        for j in img_name_list:
            if j == '.directory':
                continue
            image_array.append(read_image(os.path.join(os.getcwd(), 'dtd', 'images',class_name, j)))
            label.append(i)
...
def train_and_test(dataset_name='textures47', kernel_size=3):
    ...
    history = model.fit(dataset, label, validation_split=0.3, epochs=30, batch_size=8)
